...
function Command({ className, ...props }) {
  return (
      <CommandPrimitive
          data-slot="command"
          className={cn(
              "bg-[var(--color-popover)] text-[var(--color-popover-foreground)] flex h-full w-full flex-col "
              + "overflow-hidden rounded-[var(--radius)] border border-[var(--color-border)] shadow-lg",
              className
          )}
          {...props}
      />
...
function CommandDialog({
  ...
  return (
      <Dialog {...props}>
        ...
        >
          <Command className="[&_[cmdk-group-heading]]:text-[var(--color-muted-foreground)]
          [&_[cmdk-group-heading]]:px-2 [&_[cmdk-group-heading]]:font-medium [&_[cmdk-group]]:px-2 [&_[cmdk-group]:not([hidden])_~[cmdk-group]]:pt-0
          [&_[cmdk-input-wrapper]_svg]:h-4 [&_[cmdk-input-wrapper]_svg]:w-4 [&_[cmdk-input]]:h-11 [&_[cmdk-item]]:px-2
          [&_[cmdk-item]]:py-2.5 [&_[cmdk-item]_svg]:h-4 [&_[cmdk-item]_svg]:w-4">
            {children}
          </Command>
        </DialogContent>
      </Dialog>
  ...
  return (
      ...
      >
        ...
        <CommandPrimitive.Input
            ...
            className={cn(
                "w-full bg-transparent text-sm font-ui text-[var(--color-foreground)] placeholder:text-[var(--color-muted-foreground)] "
                + "placeholder:opacity-60 outline-none",
                className
            )}
            {...props}
        />
...
function CommandGroup({ className, ...props }) {
  return (
      <CommandPrimitive.Group
          ...
          className={cn(
              "text-[var(--color-foreground)] overflow-hidden p-1 [&_[cmdk-group-heading]]:px-2 [&_[cmdk-group-heading]]:py-1.5 "
              + "[&_[cmdk-group-heading]]:text-xs [&_[cmdk-group-heading]]:font-medium [&_[cmdk-group-heading]]:text-[var(--color-muted-foreground)]",
              className
          )}
          {...props}
      />
...
function CommandItem({ className, ...props }) {
  return (
      <CommandPrimitive.Item
          ...
          className={cn(
              "relative flex select-none items-center gap-2 rounded-[var(--radius)] px-2 py-2.5 text-sm font-ui text-[var(--color-foreground)] "
              + "transition-colors",
